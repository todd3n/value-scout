...
import { loadPaperAutoEntryStatus, loadPaperHistory, loadPaperMonitorStatus, savePaperAutoEntryStatus, savePaperHistory, type Analysis, type PaperAutoEntryStatus, type PaperMonitorStatus } from '../api/client'
import { PAPER_TRADES_KEY, TRADE_EVENTS_KEY, buildEntryDecision, formatArchivedSignalReason, formatTradeTimestamp, mergePaperHistory, paperTradePnl, paperTradeReviewDates, readLocal, resolvePaperTrade, writeLocal, type PaperTrade, type TradeEvent } from '../lib/portfolio'
...
export function PaperTradingPage({ results, freshSymbols }: { results: Analysis[]; freshSymbols: Set<string> }) {
  ...
  const [autoEntry, setAutoEntry] = useState<PaperAutoEntryStatus | null>(null)
  const [autoEntrySaving, setAutoEntrySaving] = useState(false)
  const [autoEntryError, setAutoEntryError] = useState('')
  ...
  useEffect(() => { void loadPaperAutoEntryStatus().then(setAutoEntry) }, [])
  ...
  function record(result: Analysis) {
    ...
    const reason = formatArchivedSignalReason(buyReason(result), result.maxDayDropDate ?? undefined, openedAt)
    const source = result.buyReasons?.[0]?.source || result.dropReasonSource
    const entryDecision = buildEntryDecision({ reason, source, price, targetPrice: result.bounceTarget, stopPrice: price * 0.95, dataQuality: result.dataQuality, dataAsOf: openedAt, signalDate: result.maxDayDropDate ?? undefined })
  ...
  async function toggleAutoEntry() {
    const enabled = !autoEntry?.enabled
    setAutoEntrySaving(true)
    setAutoEntryError('')
    const next = await savePaperAutoEntryStatus(enabled, 3)
    if (next) setAutoEntry(next)
    else setAutoEntryError('Inställningen kunde inte sparas. Försök igen.')
  ...
  return <main className="feature-page paper-page"><div className="feature-heading"><div><p className="eyebrow">PAPER VALIDATION LAB</p><h1>Paper trading</h1><p>Logga AI:ns köplägen som simulerade positioner. Inga riktiga order skickas. Resultaten blir bara en historik för att mäta om signalerna håller över tid.</p></div><span className="feature-badge">SIMULERADE ORDER · INGEN BROKER</span></div>
    ...
    <section className={`auto-entry-strip ${autoEntry?.enabled ? 'active' : ''}`} aria-live="polite"><span className={autoEntry?.enabled ? 'monitor-led active' : 'monitor-led'}></span><div><strong>{autoEntry?.enabled ? 'AUTOMATISKA PAPER-KÖP AKTIVA' : 'AUTOMATISKA PAPER-KÖP AVSTÄNGDA'}</strong><small>Policy B: högst 3 öppna positioner. Endast färska, liveverifierade köplägen under öppen börs med data- och riskskydd.</small></div><em>{autoEntry?.lastRunAt ? `${formatTradeTimestamp(autoEntry.lastRunAt)} · ${autoEntry.lastSummary}` : autoEntry?.lastSummary || 'Hämtar inställning…'}</em><button className="secondary-btn compact" type="button" onClick={() => void toggleAutoEntry()} disabled={autoEntrySaving}>{autoEntrySaving ? 'Sparar…' : autoEntry?.enabled ? 'Pausa auto-köp' : 'Aktivera auto-köp'}</button>{autoEntryError && <small className="auto-entry-error">{autoEntryError}</small>}</section>
