...
export function PaperTradingPage({ results }: { results: Analysis[] }) {
  ...
  function record(result: Analysis) {
    ...
    setTrades((current) => [...current, { id: `${result.symbol}-${Date.now()}`, symbol: result.symbol, name: result.name, market, reason: buyReason(result), entryPrice: price, targetPrice: result.bounceTarget, stopPrice: price * 0.95, quantity, currency: result.currency, setupLabel: result.setupLabel, openedAt: new Date().toISOString(), status: 'open', source: result.buyReasons?.[0]?.source || result.dropReasonSource }])
  }
