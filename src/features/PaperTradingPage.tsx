...
import { PAPER_TRADES_KEY, paperTradePnl, readLocal, writeLocal, type PaperTrade } from '../lib/portfolio'
...
export function PaperTradingPage({ results }: { results: Analysis[] }) {
  ...
  useEffect(() => {
    setTrades((current) => current.map((trade) => {
      ...
      const hitTarget = trade.targetPrice != null && price >= trade.targetPrice
      const hitStop = trade.stopPrice != null && price <= trade.stopPrice
      if (!hitTarget && !hitStop) return trade
      return { ...trade, status: hitTarget ? 'won' : 'lost', exitPrice: price, closedAt: new Date().toISOString() }
    }))
  }, [quotes])
  ...
  return <main className="feature-page paper-page"><div className="feature-heading"><div><p className="eyebrow">PAPER VALIDATION LAB</p><h1>Paper trading</h1><p>Logga AI:ns köplägen som simulerade positioner. Inga riktiga order skickas. Resultaten blir bara en historik för att mäta om signalerna håller över tid.</p></div><span className="feature-badge">SIMULERADE ORDER · INGEN BROKER</span></div>
    <section className="metric-grid paper-metrics panel"><div><span>Öppna trades</span><strong>{open.length}</strong></div><div><span>Stängda trades</span><strong>{closed.length}</strong></div><div><span>Träffsäkerhet</span><strong>{closed.length ? `${Math.round(wins / closed.length * 100)}%` : '—'}</strong></div><div><span>Realiserat P/L</span><strong className={closedPnl >= 0 ? 'pos' : 'neg'}>{money(closedPnl)}</strong></div><div><span>Orealiserat P/L</span><strong className={openPnl >= 0 ? 'pos' : 'neg'}>{money(openPnl)}</strong></div></section>
