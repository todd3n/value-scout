...
import { PAPER_TRADES_KEY, TRADE_EVENTS_KEY, formatTradeTimestamp, mergePaperHistory, paperTradePnl, readLocal, resolvePaperTrade, writeLocal, type PaperTrade, type TradeEvent } from '../lib/portfolio'
...
export function PaperTradingPage({ results }: { results: Analysis[] }) {
  ...
  function record(result: Analysis) {
    ...
    setTrades((current) => [...current, { id: tradeId, orderId, orderStatus: 'filled', symbol: result.symbol, name: result.name, market, reason, entryPrice: price, targetPrice: result.bounceTarget, stopPrice: price * 0.95, quantity, currency: result.currency, setupLabel: result.setupLabel, openedAt, status: 'open', source: result.buyReasons?.[0]?.source || result.dropReasonSource }])
    setEvents((current) => [...current, { id: `${tradeId}-buy`, tradeId, orderId, action: 'buy', symbol: result.symbol, name: result.name, market, quantity, price, currency: result.currency, reason, source: result.buyReasons?.[0]?.source || result.dropReasonSource, status: 'filled', occurredAt: openedAt }])
  }

  function closeTrade(trade: PaperTrade) {
    const quote = quotes[trade.symbol]
    const exitPrice = quote?.price ?? trade.entryPrice
    setTrades((current) => current.map((item) => item.id === trade.id ? { ...item, status: 'closed', orderStatus: 'closed', exitPrice, closedAt: new Date().toISOString() } : item))
  }
