...
export function PortfolioPage({ results }: { results: Analysis[] }) {
  ...
  const missingFundSymbols = rows.filter(({ holding }) => holding.type === 'fund' && quotes[holding.symbol]?.price == null).map(({ holding }) => holding.symbol)
  const exposures = rows.slice().sort((a, b) => b.metrics.marketValue - a.metrics.marketValue).slice(0, 5).map(({ holding, metrics }) => ({ symbol: holding.symbol, value: metrics.marketValue, weight: totalValue ? metrics.marketValue / totalValue * 100 : 0 }))
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      setQuotes(next)
      const missingQuotes = holdings.filter((holding) => next[holding.symbol]?.price == null)
      if (missingQuotes.length) {
        setError(missingQuotes.map((holding) => holding.type === 'fund' ? fundQuoteWarning(holding.symbol, false) : `Kursdata saknas för ticker: ${holding.symbol}. Snittpriset används tills en verifierad kurs kan hämtas.`).join(' '))
      }
  ...
  return <main className="feature-page portfolio-page">
    ...
    <section className="portfolio-tape" aria-label="Portföljstatus"><span>POSITION BOOK</span><strong>{holdings.length} INNEHAV</strong><span>STÖRSTA EXPONERING</span><strong>{largest?.holding.symbol || '—'}</strong><span>DATAKÄLLA</span><strong>YAHOO / VS RESEARCH</strong></section>
    <section className="feature-grid">
      ...
      <section className="panel summary-panel"><p className="side-label">ÖVERSIKT</p><h2>Portföljens signal</h2><div className="metric-grid"><div><span>Marknadsvärde</span><strong>{money(totalValue)}</strong></div><div><span>Resultat</span><strong className={totalPnl >= 0 ? 'pos' : 'neg'}>{money(totalPnl)}</strong></div><div><span>Avkastning</span><strong className={totalCost && totalPnl >= 0 ? 'pos' : 'neg'}>{totalCost ? pct(totalPnl / totalCost * 100) : '—'}</strong></div><div><span>Största post</span><strong>{largest?.holding.symbol || '—'}</strong></div></div><div className="exposure-list" aria-label="Största portföljexponeringar"><div className="exposure-heading"><span>TOPP-EXPONERINGAR</span><small>Vikt av marknadsvärde</small></div>{exposures.length ? exposures.map((item) => <div className="exposure-row" key={item.symbol}><span>{item.symbol}</span><i><b style={{ width: `${Math.min(100, item.weight)}%` }} /></i><strong>{item.weight.toFixed(1)}%</strong></div>) : <p className="fine">Lägg till ett innehav för att se fördelningen.</p>}</div><button className="primary-btn" type="button" onClick={runAnalysis} disabled={loading || holdings.length === 0}>{loading ? 'Analyserar portfölj…' : 'Kör AI-analys'}</button></section>
    </section>
    <section className="panel"><div className="section-heading"><div><p className="side-label">INNEHAV</p><h2>Registrerade positioner</h2></div><span>{holdings.length} poster</span></div>{missingFundSymbols.length > 0 && <p className="error-banner" role="alert">{missingFundSymbols.map((symbol) => fundQuoteWarning(symbol, false)).join(' ')}</p>}{holdings.length === 0 ? <div className="empty"><p>Din portfölj är tom. Lägg till första innehavet ovan.</p></div> : <div className="table-wrap"><table className="grid"><thead><tr><th>Ticker</th><th>Typ</th><th>Antal</th><th>Senaste kurs</th><th>Värde</th><th>Resultat</th><th></th></tr></thead><tbody>{rows.map(({ holding, metrics }) => { const hasLiveQuote = quotes[holding.symbol]?.price != null; return <tr key={holding.id}><td><strong>{holding.symbol}</strong></td><td>{holding.type === 'fund' ? 'Fond' : 'Aktie'}</td><td>{holding.quantity}</td><td>{hasLiveQuote ? money(metrics.currentPrice, holding.currency) : <span className="quote-missing">Ej verifierad · snittpris används</span>}</td><td>{money(metrics.marketValue, holding.currency)}</td><td className={metrics.pnl >= 0 ? 'pos' : 'neg'}>{money(metrics.pnl, holding.currency)} ({pct(metrics.pnlPct)})</td><td><button className="text-btn" type="button" onClick={() => { setEditingId(holding.id); setSymbol(holding.symbol); setType(holding.type); setQuantity(String(holding.quantity)); setAverageCost(String(holding.averageCost)) }}>Redigera</button><button className="icon-btn" type="button" aria-label={`Ta bort ${holding.symbol}`} onClick={() => setHoldings((current) => current.filter((item) => item.id !== holding.id))}>×</button></td></tr> })}</tbody></table></div>}</section>
